...
import { CALL_LOGS_PAGE_SIZE } from "@/stores/call-logs-store";
...
export function CallLogsPagination({
  ...
  const start =
    totalCount === 0 ? 0 : (currentPage - 1) * CALL_LOGS_PAGE_SIZE + 1;
  const end = Math.min(currentPage * CALL_LOGS_PAGE_SIZE, totalCount);
  ...
  return (
    <div className="flex flex-col gap-4 border-t border-propnex-border px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
      <p className="text-sm text-propnex-muted">
        {totalCount === 0
          ? "Showing 0 calls"
          : `Showing ${start.toLocaleString()}-${end.toLocaleString()} of ${totalCount.toLocaleString()} calls`}
      </p>
