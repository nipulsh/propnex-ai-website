...
import { LEAD_REACTIVATION_PAGE_SIZE } from "@/stores/lead-reactivation-store";
import { cn } from "@/lib/utils";

type LeadReactivationPaginationProps = {
  currentPage: number;
  totalPages: number;
  totalCount: number;
...
export function LeadReactivationPagination({
  currentPage,
  totalPages,
  totalCount,
  onPageChange,
}: LeadReactivationPaginationProps) {
  const start =
    totalCount === 0 ? 0 : (currentPage - 1) * LEAD_REACTIVATION_PAGE_SIZE + 1;
  const end = Math.min(currentPage * LEAD_REACTIVATION_PAGE_SIZE, totalCount);
  ...
  return (
    <div className="flex flex-col gap-4 border-t border-propnex-border px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
      <p className="text-sm text-propnex-muted">
        {totalCount === 0
          ? "Showing 0 leads"
          : `Showing ${start.toLocaleString()}-${end.toLocaleString()} of ${totalCount.toLocaleString()} leads`}
      </p>
